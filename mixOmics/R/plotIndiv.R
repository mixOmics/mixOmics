...
)
{
    ...
    if (style == "ggplot2"){
        ...
        for (i in levels(group)){
            if (display.names) {
                p = p +geom_point(data = subset(df, group == i),size = 0, shape = 0)+ geom_text(data = subset(df, group == i), aes(label = names), size = 0,show.legend  = F)
            } else {
        ...
        {for (i in unique(col)){
            if (display.names) {
                ...
                size = df[df$col == i , ]$cex,show.legend  = F)
            } else {
        ...
        {for (i in unique(col)){
            if (display.names) {
                ...
                aes(label = names),
                color = unique(df[df$col == i & df$Block == paste0("Block: ", blocks[1]), ]$col),
                size = unique(df[df$col == i & df$Block == paste0("Block: ", blocks[1]), ]$cex),show.legend  = F)
            } else {
                p = p + geom_point(data = subset(df, col == i),
                color = unique(df[df$col == i & df$Block == paste0("Block: ", blocks[1]), ]$col),
