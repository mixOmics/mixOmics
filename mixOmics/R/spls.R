...
...)
{
    ...
    if (any(keepX > p))
    stop("each component of 'keepX' must be lower or equal to ", p, ".")
    
    if (any(keepY > q))
    stop("each component of 'keepY' must be lower or equal to ", q, ".")
    
    if (any(keepX <1 ))
    stop("each component of 'keepX' must be higher or equal to ", 1, ".")
    
    if (any(keepY <1 ))
    stop("each component of 'keepY' must be higher or equal to ", 1, ".")
    ...
    for (h in 1:ncomp) {
        ...
        repeat {
            ...
            #selection on a (loadings.X). modified on 19/02/15 to make sure that b!=0
            if(ny!=0)
            {
                absb=abs(b)
                a=ifelse(absb>absb[which(rank(absb)==max(rank(absb)[which(rank(absb)<=(ny))]))],
    ...
    #if near.zero.var was set to TRUE, and some columns were removed, nzv is an output
