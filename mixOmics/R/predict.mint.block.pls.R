...
function(object, newdata,study.test,dist = c("all", "max.dist", "centroids.dist", "mahalanobis.dist"), multilevel = NULL, ...)
{
    ...
    for (i in 1 : J)
    {
        ...
        Y.hat[[i]] = array(Ypred, c(nrow(newdata[[i]]), ncol(Y), ncomp)) # in case one observation and only one Y, we need array() to keep it an array with a third dimension being ncomp
