...
# date: 15/02/2015: include all the testing + development (messy)
...
# ---- GIT commands (if need be) --------
# in a terminal, to add new source files:
# https://confluence.atlassian.com/display/BITBUCKET/Clone+Your+Git+Repository+and+Add+Source+Files

# got to GIT/mixomics/ to get the working git version
git --version
git status 

# see here to clone the first working directory from bitbucket
#https://confluence.atlassian.com/display/BITBUCKET/Clone+Your+Git+Repository+and+Add+Source+Files
# the command line will be something like: git clone https://klecao@bitbucket.org/klecao/mixomics.git (change username)

# BEFORE ANYTHING, DO A CHECKOUT 
#from the git directory (/Users/klecao/Documents/k.lecao/Packages/mixOmics/WorkingCopy/GIT/mixomics)
git checkout
...
# to add a new file
bash-3.2# git add mixOmics/R/perf.R 
bash-3.2# git add mixOmics/R/plot.perf.R 
bash-3.2# git add mixOmics/man/perf.Rd 
bash-3.2# git add mixOmics/man/plot.perf.Rd 

# and this is when I delete some files as well:
bash-3.2# git rm mixOmics/R/plot.valid.R
bash-3.2# git rm mixOmics/man/plot.valid.Rd
bash-3.2# git add mixOmics/NEWS

# then commit, try put an EXPLICIT message
git commit -m 'bug perf/predict resulting from changes on pls/spls 5.0-2 fixed. This is now version 5.0-3'

# then push
git push -u origin master

# then log into bitbucket and approve the changes
# -------------------------------------------
...
# ------------
# - Benoit, I think the withinVariation function should have as argument (X, design) (and not factors and rep.measures) to be consistent,
# could you please change that? # done
# - Benoit could you try fix the pheatmap function? (see section 11.3.2 below and l295: Benoit_pheatmap) # done
# - I will need to change the help files

# ---------------
# load data
# ---------------
# 
# load('multilevel_new/vac18.study.rda')
# attributes(vac18.study)
# # $names
...
# -------------
# multilevel 1 lvel: testing example to be changed in the man function
# -------------
...
res_mixomics <- multilevel(X, cond = Y, ncomp = 3,
...
for (i in names(res_mixomics)[!names(res_mixomics) %in% c("call", "sample", "name.condition", "tab.prob.gene")]){
  if (!all.equal(res_IG[[i]], res_mixomics[[i]]) == TRUE) {
    stop(paste(i, sep = "-"))
  }
}
### Dicrepancy found between IG update and mixomics
# call: ok not the same arguments
# sample: ok output present only in multilevel (mixOmics)
# name.condition: ok output missing in multilevel (update_IG) ??? should be added ???
# tab.prob.gene: ok output present only in multilevel (mixOmics)

# color for plotIndiv
stimu <- as.numeric(vac18$stimulation)
col.stimu <- c("darkblue", "purple", "green4","red3")  ##** need to put the mixOmics colors palette
col.stimu <- col.stimu[stimu]

plotIndiv(res_IG, ind.names = vac18$stimulation, col = col.stimu)
plotIndiv(res_mixomics, ind.names = vac18$stimulation, col = col.stimu)
### same colors and orders => ok
