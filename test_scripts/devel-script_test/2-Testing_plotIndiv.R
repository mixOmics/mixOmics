...
# Objective: only code for help file; testing update FB
# Latest update: 
# -----------------------------------------------------------------------------------
...
# classic style, in the Y space, is the legend far off here? see bug report
plotIndiv(nutri.res, rep.space= 'Y-variate', group = nutrimouse$genotype, 
...
# playing with style and cex size
plotIndiv(splsda.breast, ind.names = TRUE, comp = c(1, 2), plot.ellipse = TRUE, 
          style = "ggplot2", cex = c(2, 1))
plotIndiv(splsda.breast, ind.names = TRUE, comp = c(1, 2), plot.ellipse = TRUE, 
          style = "lattice", cex = c(1, 2))
...
plotIndiv(nutrimouse.sgccda1, add.legend = TRUE)
