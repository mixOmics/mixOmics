...
plotIndiv <-
  function(object,
           ...
           X.label = NULL,
           Y.label = NULL,
           show.var = TRUE, #only for PCA object
           abline.line = FALSE,
           col.per.group,
           col,
           cex,
           ...
           main="plotIndiv", # need to change title by default
           add.legend=FALSE,
           ...
           )
{
    ...
    #-- Variance explained
    if (show.var & class.object[1] %in% c("pca","spca")){
      inf=c((object$sdev)[comp1]/sum(object$sdev),(object$sdev)[comp2]/sum(object$sdev))
      inf = round(inf,2)
      X.label = paste(X.label,inf[1],sep=",         Var=")
      Y.label = paste(Y.label,inf[2],sep=",         Var=")
    }
    ...
    if (style == "ggplot2"){
      ...
      if (!add.legend) {
        ...
        p = p + guides(colour = guide_legend(override.aes = list(shape = unique(pch), size = unique(cex))))
      }
    ...
    if(style=="lattice") {
      p = xyplot(y ~ x | Block, data = df, xlab = X.label, ylab = Y.label, main = main,
                 ...
                 key = if(add.legend == TRUE) {list(space = "right", title = "Legend", cex.title = 1.5,
                                                    point = list(col = unique(col.per.group)),cex=1.3, pch = if (display.names) {97} else {unique(pch)[order(unique(group))]},text = list(levels(group)))}
                 else {NULL},
    ...
    if(style=="graphics") {
      ...
      for (k in 1 : length(x)){
        ...
        if (add.legend) {                 
            legend(par()$usr[2]+0.3,par()$usr[4] - (par()$usr[4]-par()$usr[3])/2, col = unique(levels.color), legend = levels(group), pch = if (display.names) {97} else {unique(pch)}, title = 'Legend', cex = 0.8)
        }
